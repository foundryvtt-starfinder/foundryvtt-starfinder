...
export class ItemStarfinder extends Item {
    ...
    rollAttack(options = {}) {
        ...
        itemData.hasCapacity = this.has

        rollData.item = itemData;
        const title = `Attack Roll`;

        //Warn the user if there is no ammo left
        if(itemData.data.capacity && itemData.data.capacity.value === 0)  ui.notifications.warn(game.i18n.format("STARFINDER.ItemNoUses", {name: this.data.name}));
    ...
    rollToolCheck(options = {}) {
        ...
        DiceStarfinder.d20Roll({
            ...
            title: `${CONFIG.STARFINDER.abilities[abl]} Check`,
            speaker: ChatMessage.getSpeaker({ actor: this.actor }),
