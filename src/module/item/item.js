...
export class ItemSFRPG extends Mix(Item).with(ItemActivationMixin, ItemCapacityMixin) {
    ...
    async _rollStarshipAttack(options = {}) {
        const parts = (this.actor.data.data.crew.useNPCCrew) ? ["max(@gunner.skills.pil.mod, @gunner.skills.pil.ranks)"] : ["max(@gunner.attributes.baseAttackBonus.value, @gunner.skills.pil.ranks)", "@gunner.abilities.dex.mod"];
