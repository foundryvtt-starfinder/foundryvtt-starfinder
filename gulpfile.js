...
function buildLess() {
  ...
  return gulp.src(`src/less/${name}.less`).pipe(less()).pipe(gulp.dest("dist"));
}
...
async function copyLibs() {
  ...
  try {
    await fs.copy(
      path.join(nodeModulesPath, tippyLib),
      "dist/lib/tippy/tippy.min.js"
    );
    await fs.copy(
      path.join(nodeModulesPath, tippyMap),
      "dist/lib/tippy/tippy.umd.min.js.map"
    );
    await fs.copy(
      path.join(nodeModulesPath, popperLib),
      "dist/lib/popperjs/core"
    );
...
async function cookWithOptions(options = { formattingCheck: true }) {
  ...
  conditionsRegularExpression =
    regularExpressionForFindingItemsInCache(conditionsCache);
  settingRegularExpression =
    regularExpressionForFindingItemsInCache(settingCache);
...
function formattingCheck(allItems) {
  for (const item of allItems) {
    ...
    if (data.type === "npc") {
      ...
      formattingCheckItems(data, pack, item.file, {
        checkImage: true,
        checkSource: true,
        checkPrice: true,
        checkLevel: true,
...
function formattingCheckRace(data, pack, file, options = { checkLinks: true }) {
  ...
  if (!data.name || data.name.endsWith(" ") || data.name.startsWith(" ")) {
    addWarningForPack(
      `${file}: Name is not well formatted "${data.name}.`,
      pack
    );
  }
  ...
  if (
    data.img &&
    !data.img.startsWith("systems") &&
    !data.img.startsWith("icons")
  ) {
    addWarningForPack(
      `${file}: Image is pointing to invalid location "${data.name}.`,
      pack
    );
  }
  ...
  if (!isSourceValid(source)) {
    addWarningForPack(
      `${file}: Improperly formatted source field "${source}.`,
      pack
    );
  }
  ...
  if (options.checkLinks) {
    ...
    if (result.found) {
      addWarningForPack(
        `${file}: Found reference to ${result.match} in description without link.`,
        pack
      );
    }
  }
...
function formattingCheckAlien(
  data,
  pack,
  file,
  options = { checkLinks: true, checkEcology: true }
) {
  // Validate name
  if (!data.name || data.name.endsWith(" ") || data.name.startsWith(" ")) {
    addWarningForPack(
  ...
  if (
    ...
    !data.data.attributes.hp ||
    !data.data.attributes.sp
  ) {
  ...
  if (
    data.token.img &&
    !data.token.img.startsWith("systems") &&
    !data.token.img.startsWith("icons")
  ) {
    addWarningForPack(
      `${file}: Image is pointing to invalid location "${data.name}.`,
      pack
  ...
  if (options.checkLinks === true) {
    ...
    if (conditionResult.found) {
      addWarningForPack(
        `${file}: Found reference to ${conditionResult.match} in biography without link.`,
        pack
      );
    }
    // Check biography for references to the setting
    let settingResult = searchDescriptionForUnlinkedReference(
      description,
      settingRegularExpression
    );
    if (settingResult.found) {
      addWarningForPack(
        `${file}: Found reference to ${settingResult.match} in description without link.`,
        pack
      );
    }
  }
  ...
  // Validate items
  for (i in data.items) {
    formattingCheckItems(data.items[i], pack, file, {
      checkImage: true,
      checkSource: false,
      checkPrice: false,
...
function formattingCheckItems(
  ...
  if (options.checkImage) {
    if (
      data.img && // Only validate if img is set
      !data.img.startsWith("systems") &&
      !data.img.startsWith("icons")
    ) {
      addWarningForPack(
  ...
  if (options.checkPrice) {
    ...
    if (!price || price <= 0) {
      addWarningForPack(
        `${file}: Improperly formatted armor price field "${price}.`,
        pack
      );
    }
  }
  ...
  if (options.checkLevel) {
    ...
    if (!level || level <= 0) {
      addWarningForPack(
        `${file}: Improperly formatted armor level field "${level}.`,
        pack
      );
    }
  }
  ...
  if (armor) {
    ...
    if (!validArmorTypes.includes(armorType)) {
      addWarningForPack(
        `${file}: Improperly formatted armor type field "${armorType}.`,
        pack
      );
    }
  }
  ...
  if (options.checkLinks) {
    ...
    if (description) {
      ...
      let conditionResult = searchDescriptionForUnlinkedReference(
        description,
        conditionsRegularExpression
      );
      if (conditionResult.found) {
        addWarningForPack(
          `${file}: Found reference to ${conditionResult.match} in description without link.`,
          pack
        );
      }

      // Check description for references to poisons / diseases
      let poisonResult = searchDescriptionForUnlinkedReference(
        description,
        poisonAndDiseasesRegularExpression
      );
      if (poisonResult.found) {
        addWarningForPack(
          `${file}: Found reference to ${poisonResult.match} in description without link.`,
          pack
        );
      }
    } else {
...
function formattingCheckWeapons(data, pack, file) {
  ...
  if (lowecaseName.includes("multiattack")) {
    // Should be [MultiATK]
    addWarningForPack(
      `${file}: Improperly formatted multiattack name field "${data.name}.`,
      pack
    );
  }
...
function formattingCheckVehicle(
  data,
  pack,
  file,
  ...
  if (!price || price <= 0) {
    addWarningForPack(
      `${file}: Improperly formatted vehicle price field "${armorType}.`,
      pack
    );
  }
  ...
  if (!level || level <= 0) {
    addWarningForPack(
      `${file}: Improperly formatted vehicle level field "${armorType}.`,
      pack
    );
  }
...
function searchDescriptionForUnlinkedCondition(description) {
  return searchDescriptionForUnlinkedReference(
    description,
    conditionsRegularExpression
  );
}
...
function searchDescriptionForUnlinkedReference(description, regularExpression) {
  ...
  if (matches && matches.length > 0) {
    ...
    for (let match of matches) {
      ...
      let matchedWord = description.substring(
        match["index"],
      ...
      let characterBefore = description.substring(
        characterBeforeIndex,
        characterBeforeIndex + 1
      );
      let characterAfter = description.substring(
        characterAfterIndex,
        characterAfterIndex + 1
      );
      ...
      // If potentially within the contents of a tag or surrounded by delimiting characters
        ...
        delimiterCharacters.includes(characterBefore) &&
        (delimiterCharacters.includes(characterAfter) || characterAfter === " ")
      ) {
      ...
      // Or hugging opening brackets, or the start of a comma separated list.
        ...
        (delimiterCharacters.includes(characterBefore) ||
          characterBefore === " ") &&
        delimiterCharacters.includes(characterAfter)
      ) {
        unlinkedReferenceFound = true;
      }
...
function isSourceValid(source) {
  ...
  const CoreBooksSourceMatch = [
    ...source.matchAll(/(CRB|AR|PW|COM|SOM|NS|GEM|TR) pg\. [\d]+/g),
  ];
  ...
  const AdventurePathSourceMatch = [
    ...source.matchAll(/AP #[\d]+ pg\. [\d]+/g),
  ];
  const StarfinderSocietySourceMatch = [
    ...source.matchAll(/SFS #[\d]+-[\d]+ pg\. [\d]+/g),
  ];
  const StarfinderAdventureSourceMatch = [
    ...source.matchAll(/SA:\S+ pg\. [\d]+/g),
  ];
  ...
  if (CoreBooksSourceMatch && CoreBooksSourceMatch.length > 0) {
    ...
    StarfinderSocietySourceMatch &&
    StarfinderSocietySourceMatch.length > 0
  ) {
    // ✅ formatted Starfinder Society source
    return true;
  } else if (
    StarfinderAdventureSourceMatch &&
    StarfinderAdventureSourceMatch.length > 0
  ) {
  ...
  for (let item of allItems) {
    ...
    if (itemMatch && itemMatch.length > 0) {
      for (let localItem of itemMatch) {
        ...
        packErrors[pack].push(
          `${item.file}: Using @Item to reference to '${localItemName}' (with id: ${localItemId}), @Item is not allowed in compendiums. Please use '@Compendium[sfrpg.${pack}.${localItemId}]' instead.`
        );
        cookErrorCount++;
      }
    }
    ...
    if (journalMatch && journalMatch.length > 0) {
      for (let localItem of journalMatch) {
        ...
        packErrors[pack].push(
          `${item.file}: Using @JournalEntry to reference to '${localItemName}' (with id: ${localItemId}), @JournalEntry is not allowed in compendiums. Please use '@Compendium[sfrpg.${pack}.${localItemId}]' instead.`
        );
        cookErrorCount++;
      }
    }
    ...
    if (compendiumMatch && compendiumMatch.length > 0) {
      for (let otherItem of compendiumMatch) {
        ...
        if (linkParts.length !== 3) {
          ...
          packErrors[pack].push(
            `${item.file}: Compendium link to '${link}' is not valid. It does not have enough segments in the link. Expected format is sfrpg.compendiumName.itemId.`
          );
          cookErrorCount++;
          continue;
        }
        ...
        if (system !== "sfrpg") {
          ...
          packErrors[pack].push(
            `${item.file}: Compendium link to '${otherItemName}' (with id: ${otherItemId}) is not referencing the sfrpg system, but instead using '${system}'.`
          );
          cookErrorCount++;
        }
        ...
        if (!(otherPack in compendiumMap)) {
          ...
          packErrors[pack].push(
            `${item.file}: '${otherItemName}' (with id: ${otherItemId}) cannot find '${otherPack}', is there an error in the compendium name?`
          );
          cookErrorCount++;
          continue;
        }
        ...
        if (otherItemId in compendiumMap[otherPack]) {
          ...
          let foundItem = allItems.find(
            (x) =>
              x.pack === otherPack &&
              (x.data.name == otherItemId || x.data.name == otherItemName)
          );
        ...
        if (!itemExists) {
          ...
          packErrors[pack].push(
            `${item.file}: '${otherItemName}' (with id: ${otherItemId}) not found in '${otherPack}'.`
          );
          cookErrorCount++;
        }
      }
...
exports.cookNoFormattingCheck = gulp.series(
  cookPacksNoFormattingCheck,
  clean,
  execBuild,
  postCook
);
